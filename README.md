...
> - `prj.cw/misis2024s` — основной проект, детекция штрих кодов
> - `prj.lab` — лабораторные задания
...
- `BGR → Grayscale`
- медианная фильтрация (снижение шума)
- автоматический подбор порогов Canny на основе медианы интенсивностей и коэффициента `sigma`
- `Canny → карта границ`

Дополнительно формируется визуализация “оригинал + карта границ”.

### 2. Детектирование линий преобразованием Хафа
...
cv::rectangle(img, barcode_rect, cv::Scalar(0,255,0), 2);
```

### 4. Сравнение с “идеальным” прямоугольником

Из parsed_data.json загружается разметка и на изображении наносится “идеальный” прямоугольник (белым).
